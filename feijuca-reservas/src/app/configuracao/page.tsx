...
const EXPLICACAO: Record<string, string> = {
  GOOGLE_SHEET_ID: "ID da planilha no Google Drive (está na URL da planilha).",
  GOOGLE_SERVICE_ACCOUNT_EMAIL: "E-mail da conta de serviço do Google Cloud.",
  GOOGLE_PRIVATE_KEY: "Chave privada da conta de serviço (arquivo JSON).",
  AUTH_SECRET: "Segredo para assinar o login. Gere um valor aleatório longo.",
};
...
export default function PaginaConfiguracao() {
  ...
  return (
    <main className="mx-auto w-full max-w-lg px-5 py-10">
      <h1 className="text-2xl font-extrabold">Diagnóstico do app</h1>
      <p className="mt-1.5 text-sm text-pds-muted">
        Confere se as variáveis de ambiente estão no ar e se a planilha responde.
      </p>

      {carregando ? (
        <p className="mt-8 text-sm text-pds-muted">Verificando...</p>
      ) : !saude ? (
        <p className="mt-8 text-sm text-red-300">Não foi possível consultar o diagnóstico.</p>
      ) : (
        <div className="mt-8 space-y-3">
          ...
          >
            <p className="text-sm font-bold">
              {saude.planilha
                ? `Planilha conectada (${saude.planilha.usuarios} usuários cadastrados).`
                : "Planilha não respondeu."}
            </p>
            {saude.erro ? (
            ...
            {!saude.planilha && !saude.erro && saude.faltando.length ? (
              <p className="mt-1 text-xs text-pds-muted">
                Configure as variáveis acima na Vercel e faça um novo deploy.
              </p>
            ) : null}
          </div>
        </div>
